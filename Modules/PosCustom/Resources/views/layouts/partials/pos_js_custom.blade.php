<script>
    $(document).ready(function(){
       let updateTimeflag = true;
    
    function updatetime(){
        if ($('#autoUpdateTime').is(':checked')){
    debugger;
           ...
           const currentDateTime = moment();
            ...
            if (transactionDateInput.length) {
                const dateTimePicker = transactionDateInput.data("DateTimePicker");
                
                if (dateTimePicker) {
                    dateTimePicker.date(currentDateTime);
                } else {
                    console.error('DateTimePicker is not initialized.');
                }
            } else {
                console.error('#transaction_date input element not found.');
            }
        }
     }
    ...
    function get_st_det(){
        $.ajax({
            url: "https://app.we2up.com/reports/stock-report"  + '?for=view_product&product_id=' + $('#view_product_stock_details').data('product_id'),
            dataType: 'html',
            success: function(result) {
                $('#view_product_stock_details').html(result);
                __currency_convert_recursively( $('#view_product_stock_details') );
            },
        });
            __currency_convert_recursively($(this));
    }
    $(document).on('click', '#update_contact_pos_note', function(){
        $.ajax({
            url: '/cusomter/pos_note_update',
            data: { 
                customer_id: $('#customer_id').val(),
                pos_note:$('#contact_pos_note').val()
            },
            //dataType: 'html',
            success: function(result) {
                if(result.success){
    ...
    $(document).on('click', '.last_customer_bonus', function(){
        $.ajax({
            url: '/sells/pos/get_last_customer_bonus',
            data: { 
                customer_id: $('#customer_id').val()
            },
            dataType: 'html',
    ...
    $(document).on('click', '.last_customer_sells', function(){
        debugger;
        $.ajax({
            url: '/sells/pos/get_last_customer_sells',
            data: { 
                customer_id: $('#customer_id').val()
            },
            dataType: 'html',
    ...
    $(document).on('click', '.pos_note_btn', function(){
        $.ajax({
            url: '/cusomter/pos_note_get',
            data: { 
                customer_id: $('#customer_id').val()
            },
            dataType: 'html',
    ...
    $(document).on('change', 'select#customer_id', function(){
        ...
        var url = '/payments/pay-contact-due/' + contact_id + '?type=sell';
        $('.pay_sale_due').attr('href',url);
    })
    ...
    $('#cal_earning').click(function(){
       debugger;
        var sum = 0;
        $('table#pos_table tbody tr').each(function() {
            var mul=__read_number($(this).find('input.base_unit_multiplier'));
            var row_purchase=__read_number($(this).find('input.pos_line_purchase_text'));
            sum = sum + (row_purchase * mul) ;
        });
        var final= __read_number($('input#final_total_input'));
        
            var total_earned= final  - parseFloat(sum);
            $("#earned_money").text(parseFloat(total_earned).toFixed(2));
        });
        // $("#prepaid_money").val(get_subtotal());
    });
    
    
    function installment_cal(){
            //
            var paid= $("#amount_0").val();
          
                var total_quantity = 0;
        var price_total = __read_number($("#remaining_due_for_ins"));//parseFloat();//get_subtotal();
        ...
        calculate_billing_details_ins(price_total);
        $("#total_payable_span").val(paid+price_total);
         $("#amount_0").val(paid);
         var final=__read_number($("#final_total_input"));
         var total_final=parseFloat(final)+parseFloat(paid);
         $("#final_total_input").val(total_final);
         pos_total_row();
         $("#amount_0").val(paid);
          calculate_balance_due_ins();
    ...
    $(document).ready(function() {
        ...
        //$('input#name_category').val($("#product_category option:selected").text());
        ...
        $('button#product_category').on('click', function(e) {
            global_p_category_id = $(this).val();
            $name = $(this).data('name');
            //#JCN Add in the version 6 to start from the page = 1
            $('#suggestion_page').val(1);
            //$('input#name_category').val($(this).attr('title')); //Get de name of the category
            //$('select#product_category').val(categoria_id); //To put in the select trigger the value of this button
    
            $('input#name_category').val($(this).attr('title'));
    
            var location_id = $('input#location_id').val();
            if (location_id != '' || location_id != undefined) {
                console.log('en el if button#product_category Cate Value: ' +  $(this).val() + ' Nombre: ' + $(this).attr('title') );
            ...
            get_featured_products();
            //$('.product_category').trigger('change'); //Trigger the event above     
            console.log('button#product_category Cate Value: ' +  $(this).val() + ' Nombre: ' + $(this).attr('title') );
    ...
    // on click sub category in category drawer
        $('.product_category').on('click', function(e) {
            ...
            $('input#suggestion_page').val(1);
                    ...
                    //#JCN Put the category name in the input.
                    $('input#name_category').val($(this).data('name'));
                    console.log('cate Value: ' + $(this).data('value') 
    ...
    function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
        ...
        console.log('en get_product_suggestion_list cate Value: ' +  category_id + ' Page: ' + $('input#suggestion_page').val() );
    
    
        if (url == null) {
            //#JCN the url from PosCustom/Routes/web.php
            url = '/poscustom/sells/pos/get-product-suggestion-custom';
        }
    ...
    function pos_each_row(row_obj) {
        ...
        var price_after_discount = unit_price;
        if (discount_amount > 0) {
            if (discount_type === 'fixed') {
                price_after_discount = unit_price - discount_amount;
            } else if (discount_type === 'percentage') {
                price_after_discount = unit_price - (unit_price * discount_amount / 100);
            }
        }
        ...
        var line_total = quantity * price_after_discount;
