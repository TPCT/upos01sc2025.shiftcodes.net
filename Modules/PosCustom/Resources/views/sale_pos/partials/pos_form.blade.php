@php
    ...
    ];	$contact_id = 123; // ضع هنا الـ contact_id المطلوب

	$payment_line = DB::table('transaction_payments')
    ->where('payment_for', $walk_in_customer) // استبدل `$contact_id` بالمعرف المطلوب
...
<div class="row">
	<div class="col-md-6"> {{--#JCN modify from 4 to 6--}}
		<div class="form-group">
			...
			<small class="text-danger contact_due_text"><strong>@lang('account.customer_due'):</strong> <span> {{$due}} </span></small>
		</div>
	</div>
...
<!-- زر لفتح المودال -->
<button title="عرض اخر فواتير" class="btn btn-sm btn-warning last_customer_bonus bg-navy" data-toggle="modal" data-target=".last_transtction"><i class="fas fa-dollar-sign" aria-hidden="true"></i></button>
<button type="button" class="btn btn-sm btn-primary pay_sale_due1" data-toggle="modal" data-target=".pay_contact_due_modal">
	<i class="fas fa-money-bill-alt" aria-hidden="true"></i> @lang('التحصيل')
</button>
<br><br>
<div class="row">
	<div class="col-sm-12 pos_product_div">
		...
		<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
			<thead>
				<tr>
					<th class="tex-center @if(!empty($pos_settings['inline_service_staff'])) col-md-2 @else col-md-3 @endif">	
...
<script>
    ...
    $('#pay_contact_due_form').on('submit', function (e) {
        e.preventDefault(); // منع الإجراء الافتراضي للنموذج

        // التأكد من أن البيانات ترسل للرابط الصحيح
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                location.reload(); // إعادة تحميل الصفحة إذا لزم الأمر
            },
            error: function (xhr) {
                // التعامل مع الأخطاء
